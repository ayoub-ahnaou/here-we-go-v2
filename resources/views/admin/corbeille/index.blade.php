                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach ($annonces as $annonce)
                            <tr>
                                ...
                                <td class="px-6 py-1 text-xs whitespace-nowrap text-gray-900">{{ $annonce->updated_at->diffForHumans() }}
                                </td>
                                <td class="px-6 py-1 text-xs whitespace-nowrap text-gray-500">
                                    <div class="flex space-x-2">
                                        <form action="{{ route('annonces.restore', $annonce) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="text-green-600 hover:text-green-700 underline">
                                                restore
                                            </button>
                                        </form>
                                    </div>
